...
export default function ResumePreview() {
  ...
  const showSkills =
    !pathname?.includes("/wizard/profile") &&
    !pathname?.includes("/wizard/experience");
      ...
      >
        ...
        <div className="mb-8">
          ...
          <div className="text-gray-600">
            {identity.city || <span className="text-gray-400">City, ST</span>} •{" "}
            {identity.email || (
        ...
        <div className="mb-8">
          ...
          <p className={!content.objective ? "text-gray-400" : ""}>
            {content.objective ||
              "Your career objective statement will appear here"}
          </p>
        </div>
          ...
          <div className="mb-8">
            ...
            {content.experience.length > 0 ? (
                ...
                <div key={index} className="mb-6">
                      ...
                      <h3 className="text-xl font-semibold">
                        {job.company || (
                          <span className="text-gray-400">
                            Company Name for Job #{index + 1}
                          </span>
                        )}
                      </h3>
                      <div className="text-gray-600 italic">
                        {job.title || (
                          <span className="text-gray-400">Your Title</span>
                        )}
                      </div>
                    </div>
                  ...
                  {job.responsibilities.length > 0 ? (
                    <ul className="list-disc ml-4">
                      {job.responsibilities.map((resp, i) => (
                        <li key={i} className="mb-1">
                          {resp || (
                            <span className="text-gray-400">
                              Enter responsibility #{i + 1} for Job #{index + 1}
                            </span>
                          )}
                        </li>
                      ))}
                    ...
                    <p className="text-gray-400 ml-4">
                      Add responsibilities for Job #{index + 1}
                    </p>
                  )}
                </div>
              ))
              ...
              <p className="text-gray-400">
                Your work experience entries will appear here
              </p>
            )}
          </div>
        )}
