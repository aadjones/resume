...
const getRandomUnused = (items: readonly string[], usedSet: Set<string>): string | null => {
...
const getRandomDateRange = (dateRanges: readonly string[], jobIndex: number): string => {
...
const generateResponsibilities = (responsibilities: readonly string[], usedSet: Set<string>): string[] => {
  const localUsedResponsibilities = new Set<string>();
  return Array(3).fill('').map(() => {
    let randomResp;
    do {
      randomResp = responsibilities[Math.floor(Math.random() * responsibilities.length)];
